package com.kh.practice.common.config;
...
import com.kh.practice.common.interceptor.BoardNameInterceptor;
import com.kh.practice.common.interceptor.BoardTypeInterceptor;
...
public class InterceptorConfig implements WebMvcConfigurer{
	...
	//BoardTypeInterceptor가 아직 bean등록이 안돼있어서 bean 등록해주기
	
	@Bean //개발자가 만들어서 반환하는 객체를 bean으로 등록(관리는 Spring Container에게 넘긴다)
	public BoardTypeInterceptor boardTypeInterceptor() {
		return new BoardTypeInterceptor();
		//이거 기본생성자로 BoardTypeInterceptor객체 만드는데 
		//BoardTypeInterceptor클래스에서 @RequiredArgsConstructor 사용하면 private final을 쓰면 
		// BoardTypeInterceptor 참고하기
	} 

	//게시판 이름 출력하는 인터셉터를 Bean으로 등록
	@Bean
	public BoardNameInterceptor boardNameInterceptor() {
		return new BoardNameInterceptor(); //이것만 쓰면 동작 못하고 밑의 addInterceptors에 어떤 주소에 동작할 지 작성하기
	}
