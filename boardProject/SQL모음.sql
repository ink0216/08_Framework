...
/*참조하고 있던 값 삭제되면 자식들도 다 지워버리라는 ON DELETE CASCADE 설정!!!!*/
...
COMMIT;
--채팅방 목록 조회하는 sql
SELECT CHATTING_NO --채팅방 번호
			,(SELECT MESSAGE_CONTENT FROM (
				SELECT * FROM MESSAGE M2
				WHERE M2.CHATTING_NO = R.CHATTING_NO
				ORDER BY MESSAGE_NO DESC) 
				WHERE ROWNUM = 1) LAST_MESSAGE --마지막으로 보낸 메시지(상관서브쿼리, 메인쿼리의 값이 서브쿼리에 영향 줌)
			...
			,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
				...
				R.PARTICIPANT, --참여자 (NULL이 아닌 경우) ->이 사람이 메시지 받는 대상이 된다
				R.OPEN_MEMBER --연 사람 (NULL인 경우) -> 채팅방을 만든 사람이 메시지를 받는 대상이 된다
				) TARGET_NO	
				--NVL2 : SELECT를 조회했는데 만약 NULL이 아닌 경우 참여자 번호를 TARGET_NO로 쓸거고 
				--	그럼 참여자가 메시지를 받은 대상이 된다
				--NULL이면 OPEN_MEMBER의 번호를 TARGET_NO로 쓸거다
				--5번 채팅방에서 6번 회원이 방 만들어서 1번 회원이 참여한 경우
				--5번 채팅방에 6번 회원이 만든 방 찾아라 하면 null안나옴 -> 그럼 참여자가 메시지를 받은 TARGET_NO가 된다
				--5번 채팅방에 1번 회원이 만든 방 찾아라 하면 null나옴 -> 연 사람이 메시지를 받는 사람이 된다
			,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
				WHERE R2.CHATTING_NO = R.CHATTING_NO
				AND R2.OPEN_MEMBER = #{memberNo}),
				...
				) TARGET_NICKNAME	--타겟이 참여자인지 방 만든 사람인지 어쩄그 닉네임이 무엇인가
			,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
				WHERE R2.CHATTING_NO = R.CHATTING_NO
				AND R2.OPEN_MEMBER = #{memberNo}),
		...
		--채팅방이 여러 개가 나오는데 마지막 메시지가 제일 최근에 전송된 채팅방이 가장 위에 나오게 정렬하는 ORDER BY절
...
UPDATE CUSTOMER SET CUSTOMER_NAME ='가가가'
WHERE CUSTOMER_NO=3;
COMMIT;
